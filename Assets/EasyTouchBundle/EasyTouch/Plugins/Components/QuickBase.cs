...
public class QuickBase : MonoBehaviour {
	...
	protected Vector3 GetInfluencedAxis(){
       ...
       [SerializeField] public bool isScale=false;
      [SerializeField]  public float minScale = 1;
      [SerializeField]  public float maxScale = 2;
		...
		case DirectAction.Scale:
                   ...
                   transform.localScale += localAxis * value;
                    if (isScale)
                    {
                        ...
                        float ScaleValue = Mathf.Clamp(tempScale, minScale, maxScale);
                        Vector3 v3Scale = Vector3.one * ScaleValue ;
